...
router.post("/avatar", checkAuth, async (req, res) => {
    ...
    try {
        ...
        res.json({ message: "Avatar aggiornato con successo", avatar: result.rows[0].avatar });
    } catch (error) {
...
router.get("/avatar", checkAuth, async (req, res) => {
    ...
    try {
        const result = await client.query("SELECT avatar FROM users WHERE user_id = $1", [user_id]);
