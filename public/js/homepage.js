...
}, { threshold: 0.2 }); // Considera l'elemento come visibile quando almeno il 50% è visibile

// Osserviamo entrambi i div
observerReveal.observe(revealRightElement);
observerReveal.observe(revealLeftElement);

document.getElementById("waiting-list-form").addEventListener("submit", async (e) => {
    e.preventDefault();

    const waitingListName = document.getElementById("name").value;
    const waitingListEmail = document.getElementById("email").value;
    const waitingListpreferences = document.getElementById("preferences").value;
